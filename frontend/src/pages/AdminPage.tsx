...
export default function AdminPage() {
  ...
  const reviewRep    = useMutation({ mutationFn: ({id,action,notes}:{id:string,action:string,notes?:string})=>moderationApi.reviewReport(id,{action,notes}), onSuccess:()=>qc.invalidateQueries({queryKey:['admin-reports', reportStatus]}) })
  const suspendUser  = useMutation({ mutationFn: ({id,data}:{id:string,data:any})=>moderationApi.suspendUser(id,data), onSuccess:()=>{ ok('User suspended'); qc.invalidateQueries({queryKey:['admin-reports', reportStatus]}); qc.invalidateQueries({queryKey:['mod-users']}) } })
  ...
  const banUser      = useMutation({ mutationFn: ({id,data}:{id:string,data:any})=>moderationApi.banUser(id,data), onSuccess:()=>{ ok('User banned'); qc.invalidateQueries({queryKey:['admin-reports', reportStatus]}); qc.invalidateQueries({queryKey:['mod-users']}) } })
