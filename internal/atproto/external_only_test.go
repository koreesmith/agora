...
// still produces a Bluesky record, despite BroadcastPost's own re-derived
// visibility check normally requiring "public".
func TestBroadcastPostDeliversExternalOnlyPost(t *testing.T) {
	db := testDB(t)
	defer db.Close()
	...
	username := fmt.Sprintf("agora371_bsky_%d", unique)
	...
	if err := db.QueryRow(`SELECT rkey FROM atproto_posts WHERE post_id = $1`, postID).Scan(new(string)); err != nil {
		t.Fatalf("expected the external_only post to be federated (atproto_posts row), got none: %v", err)
	}
}
